    ...
    stages {
        stage('Build Binaries & Deploy API Portal') {
            steps {
                ...
                script {
                    docker.build('ubuntu-development:1.0', '--build-arg git_personal_token=${{ secrets.JENKINS_PAT }} -f Dockerfile.development .')
                    sh 'pwd'
        ...
        stage('Run SailApiTAP') {
            steps {
                ...
                script {
                    ...
                    docker.build('ubuntu-sailtap:1.0', '--build-arg git_personal_token=${{ secrets.JENKINS_PAT }} -f Dockerfile.test .')
